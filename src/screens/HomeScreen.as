package screens
{
	...
	public class HomeScreen extends PanelScreen
	{
		...
		{
			var request:URLRequest = new URLRequest(Firebase.FIREBASE_SELECT_URL+Firebase.LOGGED_USER_DATA.localId+
				'.json?auth='+Firebase.LOGGED_USER_DATA.idToken+'&orderBy="due_date"');
		...
		private function tasksLoaded(event:flash.events.Event):void
		{
			...
			{
				tasksArray.push({id:key , title:rawData[key].title, description:rawData[key].description, due_date:rawData[key].due_date, start_date:rawData[key].start_date});
			}
		...
		private function deleteTask():void
		{
			...
			var request:URLRequest = new URLRequest(Firebase.FIREBASE_DELETE_URL+Firebase.LOGGED_USER_DATA.localId+
				"/"+tasksList.selectedItem.id+".json?auth="+Firebase.LOGGED_USER_DATA.idToken);
		...
		private function updateEmail():void
		{
			...
			{
				if(emailInput.text == ""){
					alert = Alert.show("New Email Address is required.", "Error", new ListCollection(
						[
							{ label: "OK"}
						]) );					
				} else {
					...
					updateEmailLoader.addEventListener(flash.events.Event.COMPLETE, function():void
					{
						...
						Firebase.LOGGED_USER_DATA.email = emailInput.text;
						...
						alert = Alert.show("Email Address successfully updated.", "Success", new ListCollection(
							[
								{ label: "OK"}
							]) );														
					});
		...
		{		
			...
			updateButton.addEventListener(starling.events.Event.TRIGGERED, function():void
			{
				if(passwordInput.text == ""){
					alert = Alert.show("New Password is required.", "Error", new ListCollection(
						[
							{ label: "OK"}
						]) );					
				} else {
					...
					updatePasswordLoader.addEventListener(IOErrorEvent.IO_ERROR, onError);
